...
def generate_ticket_id():
    ...
    if not last_booking:
        return 'TK00000001'
    last_id = int(last_booking.ticket_id[2:])
    new_id = last_id + 1
    return f'TK{str(new_id).zfill(8)}'
...
class Booking(models.Model):
    ...
    trip_id = models.CharField(max_length=10)  # Assumed to be validated through interservice call
    traveller_name = models.CharField(max_length=255)
    traveller_number = models.CharField(
        max_length=10,
        validators=[RegexValidator(regex='^\d{10}$', message='Traveller number must be 10 digits')]
